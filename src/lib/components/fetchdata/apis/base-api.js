...
import appConfig from '$lib/components/fetchdata/apis/ghin/config/index';
import { getUserToken, updateUrl } from '$lib/components/fetchdata/apis/ghin/utils/api-helper';
...
class BaseApi {
	...
	httpPost(url, data = null, token = undefined) {
		return axios.post(url, this.appendSourceToData(data), this.getAuthHeader(token));
	}

	httpPatch(url, data = null) {
		return axios.patch(url, this.appendSourceToData(data), this.getAuthHeader());
	}
