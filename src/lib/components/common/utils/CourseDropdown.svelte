<script>
  ...
  import { groups, course, courseData, playersInLineup, allPlayersInTable, teamTables, teeTimeCount, teesSelected, showChangeTees, group } from '$lib/store';
  ...
  function handleChange(e) {
    ...
    if ($course !== '') {
      $teamTables = updateTeamTables($teesSelected[$course]);
      $playersInLineup = updatePlayersInLineup($teesSelected[$course]);
    }
  }
</script>

    <select name='course' bind:value={$course} onChange={handleChange}>
      <option key={'0'} value=''>
        Select Course
      </option>
